...
; Modified by RJS 7/26/06 to return cal_version

; Modified by RJS 8/6/09 to return NOTE, WARNING, CAUTION, DANGER, or SEVERE Messages

; Corrected bug that assigned wrong reference orbit to orb_ref - RJS 8/6/09
...
orb_ref = lonarr(n_epochs)
cal_msg = strarr(n_epochs)
...
while not EOF(lu) and n_epoch lt n_epochs do begin
  ...
  m = strpos(s,'# NOTE:')
  if (m eq 0) then cal_msg(n_epoch)=strmid(s,2)
  m = strpos(s,'# CAUTION:')
  if (m eq 0) then cal_msg(n_epoch)=strmid(s,2)
  m = strpos(s,'# WARNING:')
  if (m eq 0) then cal_msg(n_epoch)=strmid(s,2)
...
;
    if (orb eq 1787) then orb = 1795L
    if (orb eq 1798) then orb = 1796L
    orb_ref(n_epoch)=orb
  endif
  m = strpos(strlowcase(s),'# reference epoch')
  if m ge 0 then begin
;
;   Over-ride orb_ref
;
    m1 = strpos(strlowcase(s),'orbit') + 6
    reads,strmid(s,m1,5),orb
    mm = strpos(strlowcase(s),'end of orbit')
    if (mm gt 0) then orb = orb+1L
    orb_ref(n_epoch)=orb
  endif
...
bmsg = where (strcompress(cal_msg,/remove_all) ne '', nmsg)
if (nmsg gt 0) then retval= create_struct(retval,'message',cal_msg)
...
; Modified by RJS 07/26/06 to reset do_recal and no_recal keywords, depending on 
;     whether or not data are past end of calibration file. Also change severity
;     of warning if do_recal set

; Added cal_version keyword - RJS 07/26/06

; Added option to output messages from calibration file - RJS 08/05/09
...
if (nnt gt 0 ) then begin
    ...
    tmp=smooth(magx_fix,7 < (n_elements(magx_fix)-1L))
    tmp=[magx_fix(no),tmp(nt(nv))]
    magx_fix=tmp(t_sort)

    tmp=smooth(magy_fix,7 < (n_elements(magy_fix)-1L))
    tmp=[magy_fix(no),tmp(nt(nv))]
    magy_fix=tmp(t_sort)

    tmp=smooth(magz_fix,7 < (n_elements(magz_fix)-1L))
...
;
;       set do_recal flag, unless over-ridden by no_recal
;
      ...
      if (keyword_set(do_recal)) then begin
         no_recal=0
      endif else begin
         do_recal=0
         no_recal=1
      endelse
      if (norb gt 0) then begin
;
...
bmess = where (tag_names(cal_hist) eq 'MESSAGE', nmess)

if (nmess gt 0) then begin
  if (nb gt 0) then cal_mess = cal_hist.message[nb-1]
endif else cal_mess = ''
...
; Modified by RJS 06/29/09 to use steps in the spin axis data to better
; constrain the time interval of the torquer step

; Bug fix by RJS 06/03/10 - error in dbz_dc test
...
if (ny gt 0) then begin
   ...
   msk(by)=1
   bm = where (msk eq 0, nm)
   smth = smooth(dbz_dc(bm),21 < nm)
...
;    added extra step to also use the dbz - dbx correlation  filter

     if (nz gt 0) then begin
       for nn = 0,nz-1L do begin
         bk = where (bt eq bz[nn], nk)
         if (nk eq 0) then bz[nn]=-1
       endfor
       bk = where (bz ge 0, nk)
       if nk gt 0 then bz = bz[bk] else nz = 0 
     endif
...
;    bug fix by RJS 06/03/10 - need to skip to end of loop if nz = 0

     if (nz gt 0) then begin
       zfact=ft(1)

;      additional check - delete all steps that total less than 100 nT in dbz_dc
...
; first step -- make a time series that only has the jumps

        xl=0.d0
        yl=0.d0
        zl=0.d0
        ttorq_=[spinfit.time[0]]
        xtorq_=[xtorq[0]]
        ytorq_=[ytorq[0]]
        ztorq_=[ztorq[0]]
        ...
        n = 0L
        while n lt n_elements(bz) do begin
          if dn[n] ne 1 then begin
            ttorq_=[ttorq_,spinfit.time[bgood[bz[n]]]]
            xtorq_=[xtorq_,xtorq[bgood[bz[n]]]]
            ytorq_=[ytorq_,ytorq[bgood[bz[n]]]]
            ztorq_=[ztorq_,ztorq[bgood[bz[n]]]]
            ttorq_=[ttorq_,spinfit.time[bgood[bz[n]+1L]]]
            xtorq_=[xtorq_,xtorq[bgood[bz[n]+1L]]]
            ytorq_=[ytorq_,ytorq[bgood[bz[n]+1L]]]
            ztorq_=[ztorq_,ztorq[bgood[bz[n]+1L]]]
          endif else begin
            nll = n_elements(ttorq_)-1L
            ttorq_[nll]=[spinfit.time[bgood[bz[n]+1L]]]
            xtorq_[nll]=[xtorq[bgood[bz[n]+1L]]]
            ytorq_[nll]=[ytorq[bgood[bz[n]+1L]]]
            ztorq_[nll]=[ztorq[bgood[bz[n]+1L]]]
          endelse
          n = n + 1L
        endwhile
        nll = n_elements(spinfit.time)-1L
        ttorq_=[ttorq_,spinfit.time[nll]]
        xtorq_=[xtorq_,xtorq[nll]]
        ytorq_=[ytorq_,ytorq[nll]]
        ztorq_=[ztorq_,ztorq[nll]]

; next step -- refine the time tags so that more closely match the high resolution data

        bzro = where(ztorq_[1:*]-ztorq_[0:*] ne 0.d0, nzro)

        if (nzro gt 0) then begin
          for n = 0,nzro-1 do begin
             t1 = 2.d0*ttorq_[bzro[n]]-ttorq_[bzro[n]+1]
             t2 = 2.d0*ttorq_[bzro[n]+1]-ttorq_[bzro[n]]
             tt = where (mag.t ge t1 and mag.t le t2, nt)
             if (nt gt 0) then begin
               tchk = mag.t[tt]
               zchk = mag.z[tt]
               dz=abs(zchk[1:*]-zchk[0:*])
               mx = max(dz,nmx)
               tl = tchk[nmx]
               th = tchk[nmx+1]
               nv = 3
               while nv le 10 do begin
                 bchk = where (dz gt nv*median(dz), nchk)
                 if (nchk gt 1) then begin
                   dbchk = where (bchk[1:*]-bchk[0:*] eq 1, nbchk)
                   if (nbchk eq nchk-1) then begin
                     tl=tchk[bchk[0]]
                     th=tchk[bchk[nbchk]+1]
                     nv = 20
                   endif
                 endif else begin
                   nv = 20
...
;       Now using piece wise interpolation
        ...
        dd = [0.d0,xtorq_(1:*)-xtorq_(0:*)]
        bd = where (dd ne 0.d0, nd)
        if (nd gt 0) then begin
           tl=mag.t(0)-1.d0
           for nn = 0, nd-1L do begin
              bprev = where (mag.t gt tl and mag.t lt ttorq_(bd(nn)-1L),nprev)
              if (nprev gt 0) then begin 
                 xyz(bprev)=xyz(bprev)+xtorq_(bd(nn)-1L)
                 tl = mag.t(bprev(nprev-1L))
              endif
              bfix = where (mag.t ge ttorq_(bd(nn)-1L) and  $
              mag.t lt ttorq_(bd(nn)), nfix)
              if (nfix gt 0) then begin
                 dtorqdt=(xtorq_(bd(nn))-xtorq_(bd(nn)-1L))/(ttorq_(bd(nn))-ttorq_(bd(nn)-1L))
                 xyz(bfix)=xtorq_(bd(nn)-1L)+dtorqdt*(mag.t(bfix)-ttorq_(bd(nn)-1L))
                 tl = mag.t(bfix(nfix-1L))
              endif
           endfor
           blast = where (mag.t gt tl, nlast)
           if (nlast ne 0) then xyz(blast)=xyz(blast)+xtorq_(n_elements(xtorq_)-1L)
        endif
...
ntmp = n_elements(frq_tmp) -1L
frq_sm=smooth(smooth(frq_tmp,nsm1 < ntmp,/edge_truncate),nsm2 < ntmp,/edge_truncate)
...
ntmp = n_elements(frq_tmp) -1L
if (nsm1 gt 20) then frq_tmp=smooth(frq_tmp,15 < ntmp,/edge_truncate)        
frq_sm=smooth(smooth(frq_tmp,nsm1 < ntmp,/edge_truncate),nsm2 < ntmp,/edge_truncate)
dff = frq_tmp-frq_sm
dff = dff - smooth(smooth(dff,nsm3 < ntmp,/edge_truncate),nsm3 < ntmp,/edge_truncate)
res = sqrt(nsm3*smooth(dff*dff,nsm3 < ntmp,/edge_truncate))
wt = 1.d0 - res*100.d0
wt_raw=wt
...
if (keyword_set(is_sun)) then begin
   ...
   ; return.  If this is old, single-user SDT, "sdt_idx" is returned
   ; as 255 and we handle the call in the old way.
   sdt_idx1 = get_sdt_run_idx()

   prog = getenv('FASTBIN') + '/showDQIs'
   if ((sdt_idx1 GE 0) AND (sdt_idx1 LT 100)) then begin
       if (sdt_idx1 GE 10) then begin
           sidstr = string(sdt_idx1, format='(I2)')
       endif else begin
           sidstr = string(sdt_idx1, format='(I1)')
       endelse
       spawn, [prog, sidstr], result, /noshell
   endif else begin
       spawn, prog, result, /noshell
...
nwt = n_elements(wt) - 1L
wt = 2.d0*smooth(wt,nsm2 < nwt,/edge_truncate)
bb=where (wt gt 1., nb)
...
if (necl gt 0) then begin
   ...
   nhuh = n_elements(frq_huh) - 1L
   if (nsm1 gt 20) then frq_huh=smooth(frq_tmp(bgood),15 < nhuh,/edge_truncate)        
   frq_huh=smooth(smooth(frq_huh,nsm1 < nhuh,/edge_truncate),nsm2 < nhuh,/edge_truncate)
...
frq_sm = frq_sm + wt*smooth(frq_tmp-frq_sm,nsm3 < ntmp,/edge_truncate)
...
ndel = n_elements(del_p) - 1L
if (nsm1 gt 20) then del_p=smooth(del_p,15 < ndel,/edge_truncate)
del_p_sm=smooth(smooth(del_p,nsm1 < ndel,/edge_truncate),nsm2 < ndel,/edge_truncate)
...
add_back = smooth(smooth(del_p,nsm4 < ndel,/edge_truncate),nsm4 < ndel,/edge_truncate)- $
           smooth(smooth(del_p,nsm1 < ndel,/edge_truncate),nsm2 < ndel,/edge_truncate)
...

function set_dipole_orient,req_epoch,use_igrf7=use_igrf7
...
; use_igrf7 keyword allows use of either IGRF version 7 (1995)
; or IGRF version 10 (2005) [Default]

if (keyword_set(use_igrf7)) then begin

; igrf version 7 (1995)

  print,''
  print,'Using IGRF version 7 (1995)'
  print,''
...
; igrf version 11 (2010)

  print,''
  print,'Using IGRF version 11 (2010)'
  print,''

  igrf_dip={ $
  epoch:[    1945.d0,    1950.d0,    1955.d0,    1960.d0,    1965.d0, $
             1970.d0,    1975.d0,    1980.d0,    1985.d0,    1990.d0, $
             1995.d0,    2000.d0,    2005.d0,    2010.d0,    2015.d0], $
  g10:  [  -30594.d0,  -30554.d0,  -30500.d0,  -30421.d0,  -30334.d0, $
           -30220.d0,  -30100.d0,  -29992.d0,  -29873.d0,  -29775.d0, $
           -29692.d0, -29619.4d0,-29554.63d0, -29496.5d0,     11.4d0], $
  g11:  [   -2285.d0,   -2250.d0,   -2215.d0,   -2169.d0,   -2119.d0, $
            -2068.d0,   -2013.d0,   -1956.d0,   -1905.d0,   -1848.d0, $
            -1784.d0,  -1728.2d0, -1669.05d0,  -1585.9d0,     16.7d0], $
  h11:  [    5810.d0,    5815.d0,    5820.d0,    5791.d0,    5776.d0, $
             5737.d0,    5675.d0,    5604.d0,    5500.d0,    5406.d0, $
             5306.d0,   5186.1d0,  5077.99d0,   4945.1d0,    -28.8d0] } 
...

function set_igrf_coefficients,igrf_vers=igrf_vers

; set up a structure containing the IGRF coefficients

; igrf_vers keyword added (only version 7 and 11 implemented)

if (not keyword_set(igrf_vers)) then igrf_vers = 11
...
; next two epochs are IGRF 7th Generation (revised 1995)
...
; next four epochs are IGRF 11th Generation (revised 2010)

; dgrf95

g_arr=[     -29692.d0, -1784.d0, -2200.d0,  3070.d0,  1681.d0, $
              1335.d0, -2267.d0,  1249.d0,   759.d0,   940.d0, $
               780.d0,   290.d0,  -418.d0,   122.d0,  -214.d0, $
               352.d0,   235.d0,  -118.d0,  -166.d0,   -17.d0, $
                68.d0,    67.d0,    68.d0,  -170.d0,    -1.d0, $
                19.d0,   -93.d0,    77.d0,   -72.d0,     1.d0, $
                28.d0,     5.d0,     4.d0,     8.d0,    -2.d0, $
                25.d0,     6.d0,    -6.d0,    -9.d0,   -14.d0, $
                 9.d0,     6.d0,    -5.d0,    -7.d0,     4.d0, $
                 9.d0,     3.d0,   -10.d0,     8.d0,    -8.d0, $
                -1.d0,    10.d0,    -2.d0,    -8.d0,    -3.d0, $
                -6.d0,     2.d0,    -4.d0,    -1.d0,     4.d0, $
                 2.d0,     2.d0,     5.d0,     1.d0,     0.d0, $
                 0.d0,     0.d0,     0.d0,     0.d0,     0.d0, $
                 0.d0,     0.d0,     0.d0,     0.d0,     0.d0, $
                 0.d0,     0.d0,     0.d0,     0.d0,     0.d0, $
                 0.d0,     0.d0,     0.d0,     0.d0,     0.d0, $
...
h_arr=[          0.d0,  5306.d0,     0.d0, -2366.d0,  -413.d0, $
                 0.d0,  -262.d0,   302.d0,  -427.d0,     0.d0, $
               262.d0,  -236.d0,    97.d0,  -306.d0,     0.d0, $
                46.d0,   165.d0,  -143.d0,   -55.d0,   107.d0, $
                 0.d0,   -17.d0,    72.d0,    67.d0,   -58.d0, $
                 1.d0,    36.d0,     0.d0,   -69.d0,   -25.d0, $
                1.1d0,    -0.3d0,     0.8d0,    -0.2d0,     0.4d0, $
                0.0d0,     0.4d0,    -0.3d0,    -0.3d0  ]

h_arr=[         0.0d0,  4945.1d0,     0.0d0, -2707.7d0,  -575.4d0, $
                0.0d0,  -160.5d0,   251.7d0,  -536.8d0,     0.0d0, $
              286.4d0,  -211.2d0,   164.4d0,  -309.2d0,     0.0d0, $
               44.7d0,   188.9d0,  -118.1d0,     0.1d0,   100.9d0, $
                0.0d0,   -20.8d0,    44.2d0,    61.5d0,   -66.3d0, $
                3.1d0,    54.9d0,     0.0d0,   -57.8d0,   -21.2d0, $
                6.6d0,    24.9d0,     7.0d0,   -27.7d0,    -3.4d0, $
                0.0d0,    10.9d0,   -20.0d0,    11.9d0,   -17.4d0, $
               16.7d0,     7.1d0,   -10.8d0,     1.7d0,     0.0d0, $
              -20.5d0,    11.6d0,    12.8d0,    -7.2d0,    -7.4d0, $
                8.0d0,     2.2d0,    -6.1d0,     7.0d0,     0.0d0, $
                2.8d0,    -0.1d0,     4.7d0,     4.4d0,    -7.2d0, $
               -1.0d0,    -4.0d0,    -2.0d0,    -2.0d0,    -8.3d0, $
                0.0d0,     0.1d0,     1.7d0,    -0.6d0,    -1.8d0, $
                0.9d0,    -0.4d0,    -2.5d0,    -1.3d0,    -2.1d0, $
               -1.9d0,    -1.8d0,     0.0d0,    -0.8d0,     0.3d0, $
                2.2d0,    -2.5d0,     0.5d0,     0.6d0,     0.0d0, $
                0.1d0,     0.3d0,    -0.9d0,    -0.2d0,     0.8d0, $
                0.0d0,    -0.8d0,     0.3d0,     1.7d0,    -0.6d0, $
               -1.2d0,    -0.1d0,     0.5d0,     0.1d0,     0.5d0, $
                0.4d0,    -0.2d0,    -0.5d0,    -0.8d0  ]

igrf10={epoch:2010.d0,earth_rad:6371.2d0,n_coeffs:13,gs:g_arr,hs:h_arr}

; igrf15s

g_arr=[        11.4d0,   16.7d0,  -11.3d0,   -3.9d0,    2.7d0, $
                1.3d0,   -3.9d0,   -2.9d0,   -8.1d0,   -1.4d0, $
                2.0d0,   -8.9d0,    4.4d0,   -2.3d0,   -0.5d0, $
                0.5d0,   -1.5d0,   -0.7d0,    1.3d0,    1.4d0, $
               -0.3d0,   -0.3d0,   -0.3d0,    1.9d0,   -1.6d0, $
               -0.2d0,    1.8d0,    0.2d0,   -0.1d0,   -0.6d0, $
                1.4d0,    0.3d0,    0.1d0,   -0.8d0,    0.4d0, $
               -0.1d0,    0.1d0,   -0.5d0,    0.3d0,   -0.3d0, $
                0.3d0,    0.2d0,   -0.5d0,    0.2d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
...
h_arr=[         0.0d0,  -28.8d0,    0.0d0,  -23.0d0,  -12.9d0, $
                0.0d0,    8.6d0,   -2.9d0,   -2.1d0,    0.0d0, $
                0.4d0,    3.2d0,    3.6d0,   -0.8d0,    0.0d0, $
                0.5d0,    1.5d0,    0.9d0,    3.7d0,   -0.6d0, $
                0.0d0,   -0.1d0,   -2.1d0,   -0.4d0,   -0.5d0, $
                0.8d0,    0.5d0,    0.0d0,    0.6d0,    0.3d0, $
               -0.2d0,   -0.1d0,   -0.8d0,   -0.3d0,    0.2d0, $
                0.0d0,    0.0d0,    0.2d0,    0.5d0,    0.4d0, $
                0.1d0,   -0.1d0,    0.4d0,    0.4d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
                0.0d0,    0.0d0,    0.0d0,    0.0d0,    0.0d0, $
...
igrf15s={epoch:2015.d0,earth_rad:6371.2d0,n_coeffs:8,gs:g_arr,hs:h_arr}

if (igrf_vers eq 7) then begin
  ...
  epoch=[dgrf45.epoch,dgrf50.epoch,dgrf55.epoch,dgrf60.epoch, $
         dgrf65.epoch,dgrf70.epoch,dgrf75.epoch,dgrf80.epoch, $
         dgrf85.epoch,dgrf90.epoch,dgrf95.epoch,dgrf00.epoch, $
         dgrf05.epoch,igrf10.epoch,igrf15s.epoch]

  earth=[dgrf45.earth_rad,dgrf50.earth_rad,dgrf55.earth_rad,dgrf60.earth_rad, $
         dgrf65.earth_rad,dgrf70.earth_rad,dgrf75.earth_rad,dgrf80.earth_rad, $
         dgrf85.earth_rad,dgrf90.earth_rad,dgrf95.earth_rad,dgrf00.earth_rad, $
         dgrf05.earth_rad,igrf10.earth_rad,igrf15s.earth_rad]

  n_coef=[dgrf45.n_coeffs,dgrf50.n_coeffs,dgrf55.n_coeffs,dgrf60.n_coeffs, $
          dgrf65.n_coeffs,dgrf70.n_coeffs,dgrf75.n_coeffs,dgrf80.n_coeffs, $
          dgrf85.n_coeffs,dgrf90.n_coeffs,dgrf95.n_coeffs,dgrf00.n_coeffs, $
          dgrf05.n_coeffs,igrf10.n_coeffs,igrf15s.n_coeffs]

  gs=transpose([[dgrf45.gs],[dgrf50.gs],[dgrf55.gs],[dgrf60.gs], $
                [dgrf65.gs],[dgrf70.gs],[dgrf75.gs],[dgrf80.gs], $
                [dgrf85.gs],[dgrf90.gs],[dgrf95.gs],[dgrf00.gs], $
                [dgrf05.gs],[igrf10.gs],[igrf15s.gs]])

  hs=transpose([[dgrf45.hs],[dgrf50.hs],[dgrf55.hs],[dgrf60.hs], $
                [dgrf65.hs],[dgrf70.hs],[dgrf75.hs],[dgrf80.hs], $
                [dgrf85.hs],[dgrf90.hs],[dgrf95.hs],[dgrf00.hs], $
                [dgrf05.hs],[igrf10.hs],[igrf15s.hs]])
...

pro get_new_igrf,no_store_old=no_store_old,use_igrf7=use_igrf7
...
igrf_vers = 11
if (keyword_set(use_igrf7)) then igrf_vers=7

igrf=set_igrf_coefficients(igrf_vers=igrf_vers)
