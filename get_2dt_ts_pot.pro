...
;-
pro get_2dt_ts_pot,funct,get_dat, $
	T1=t1, $
	T2=t2, $
	ENERGY=en, $
	ERANGE=er, $
...
if keyword_set(sc_pot) then begin
	if routine EQ 'get_fa_ees_ts' or routine EQ 'get_fa_eeb_ts' then begin
			...
			sc_pot_min=min(abs(dat(0).time-sc_pot.x),ind)
			if -1.*sc_pot.y(ind) GT en(0) then begin
				
				energy_sc_pot=[-1.*sc_pot.y(ind),en(1)]
			endif else begin
			...
			sc_pot_min=min(abs(dat(0).time-sc_pot.x),ind)
			if sc_pot.y(ind) GT en(0) then begin
				
				energy_sc_pot=[sc_pot.y(ind),en(1)]
			endif else begin
...
sum = call_function(funct,dat(0),ENERGY=energy_sc_pot,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins)
...
if (dat(arr_idx).valid eq 1) then begin
	...
	if keyword_set(sc_pot) then begin
	if routine EQ 'get_fa_ees_ts' or routine EQ 'get_fa_eeb_ts' then begin
		sc_pot_min=min(abs(dat(arr_idx).time-sc_pot.x),ind)
		if -1.*sc_pot.y(ind) GT en(0) then begin
			energy_sc_pot=[-1.*sc_pot.y(ind),en(1)]
		endif else begin
			energy_sc_pot=en
	...
	energy_sc_pot=en
endelse
;print,'energy_sc_pot',energy_sc_pot
;print,time_to_str(dat(arr_idx).time,/ms)	
	
	sum = call_function(funct,dat(arr_idx),ENERGY=energy_sc_pot,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins)
	data(n,*) = sum
