...
;          cal_version Named variable to store calibration date and version number
;
...
;          do_tweak  If set tweak the gains and attitude, default action if data
;                    past the end of the calibration file
;          no_tweak  If set don't tweak the gains and attitude, default action if
;                    data within time span of the calibration file
...
;          use_igrf7 If set use the 1995 version of the IGRF model (version 7),
;                    otherwise use the 2005 model (IGRF version 10). Only applies
;                    if old_igrf not set
;
; OLD KEYWORDS:
...
;
;          More detailed:
;          ucla_mag_despin,tw_mat=tw_mat,orbit=orbit,spin_axis=spin_axis,delta_phi=delta_phi,cal_version=cal_version/query
;
; OUTPUT: 
;          tplot variables:
;
...
;              care of cases where nadir data is used in sunlight (!)
...
;
;      Version 3.9 - Use IGRF Version 9 (2003) - RJS 11/19/04
;           Added USE_IGRF7 keyword
;           Modified GET_NEW_IGRF and SET_DIPOLE_ORIENT to use either 
;             IGRF9 (2003) [default] or IGRF7 (1995)
;
;      Version 3.9a - Use IGRF Version 10 (2005) - RJS 6/13/05
;
;      Version 4.0 - New version release
;           Added CAL_VERSION keyword - RJS 7/26/06 
;           quadratic extrapolation for RA predict after end of cal file - RJS 18/1/07
;           Included DO_TWEAK and NO_TWEAK keywords - RJS 7/26/06
;           check array dimensions in smooth - RJS 6/29/09
;           Sharpened torquer corrections based on steps in spin-axis data - RJS 6/29/09
;           Hard-coded FDF polynomial to End of Mission (EOM) - RJS 6/29/09
;           Read and echo calibration file messages - RJS 8/6/09
;
;      Version 4.1 - Bug Fix and IGRF update
;           Fixed GET_TORQUER_MAG bug - RJS 6/3/10
;           Updated IGRF to version 11 (2010) - RJS 6/8/10
;
;           Following changes deferred to next version(s)
;           Corrections for bad time tags at rate changes - TBD
;           Implementation of DO_TWEAK keyword - TBD
;           Modified torquer calculation - TBD
;             Use large negative X-offset as additional flag for torquer ops
;           Return new coupling matrix and attitude data for possible cal file - TBD
;           Extend flags for cdf production - TBD
;           Revise read calibration file to quickly find desired records - TBD
;
;      Version 4.2 - Bug Fix
;           Fixed spawn of showDQIs in "ucla_mag_lib.pro" - JBV 5/22/11
;           This was required only for multi-user SDT and only requires
;           code modification in "ucla_mag_lib.pro" -
;           "ucla_mag_despin.pro" is not affected, other than the
;           update of "code_version".
;
;-
...
if (keyword_set(use_igrf7)) then use_igrf7 = 1 else use_igrf7 = 0

if (keyword_set(do_tweak)) then do_recal = 1 else do_recal = 0
if (keyword_set(no_tweak)) then no_recal = 1 else no_recal = 0

; Until calibration tweaker set up force no_recal=1, do_recal = 0
...
; temporary: tplot store of spinfit values - verify torquer calculations?

;store_data,'SPIN_BXDC',data={x:spinfit.time,y:spinfit.bx_dc}
;store_data,'SPIN_BYDC',data={x:spinfit.time,y:spinfit.by_dc}
;store_data,'SPIN_BZDC',data={x:spinfit.time,y:spinfit.bz_dc}
;store_data,'SPIN_BYBX',data={x:spinfit.time,y:spinfit.by_bx}
;store_data,'SPIN_BZBX',data={x:spinfit.time,y:spinfit.bz_bx}
;store_data,'SPIN_BYPH',data={x:spinfit.time,y:spinfit.phase_by}
;store_data,'SPIN_BZPH',data={x:spinfit.time,y:spinfit.phase_bz}
  ...
  for n = 0L,n_elements(phs),nfix do begin
    ...
    if (nkeep gt 0) then begin
      ...
      for m=0,2 do begin
         ...
         tmp_tmp = [tmp(bs),tmp(bs)]
         ntmp_tmp = n_elements(tmp_tmp)-1L
         res_sm=smooth(tmp_tmp,nsmooth < ntmp_tmp)
...
; End of mission FDF - determined 06/29/09

tref_eom=str_to_time('1996-08-26/22:00:00')
tlast_eom=str_to_time('2008-07-15/00:00:00')
ref_ra_eom=-95.755315d0
trend_ra_eom=-5.3395729d-06
second_ra_eom=-7.4718387d-16
poly_eom=dblarr(9)
poly_eom[0]= 1.4771940d0
poly_eom[1]=-1.7863235d-08
poly_eom[2]=-2.8695771d-16
poly_eom[3]= 1.5781347d-23
poly_eom[4]=-1.5234633d-31
poly_eom[5]= 5.8042249d-40
poly_eom[6]=-9.0376352d-49
poly_eom[7]= 2.4152649d-58
poly_eom[8]= 4.7136539d-67
ref_dec_eom=-10.5211d0
...
; over-ride the fdf_predict if it has a last_time earlier than the EOM FDF

use_eom = 0
if (n_tags(fdf_predict) eq 0) then begin
  use_eom=1
endif else begin
  if fdfpre_last lt tlast_eom then use_eom=1
endelse

if (use_eom) then begin
  if (develop) then print,'USING HARD-CODED EOM FDF'
  fdf_predict={ref_time:tref_eom,last_time:tlast_eom,$
  ref_ra:ref_ra_eom,trend_ra:trend_ra_eom,second_ra:second_ra_eom,$
  poly:poly_eom,$
  ref_dec:ref_dec_eom}
  fdfpre_ref = fdf_predict.ref_time
  fdfpre_last = fdf_predict.last_time
...
; linearly extrapolate to reference time
...
; use linear extrapolation only for out of range

    if (tref lt fdfpre_ref) then tref2 = fdfpre_ref
    if (tref gt fdfpre_last) then tref2 = fdfpre_last
endif
   ...
   if (ntag gt 0) then begin
     ...
     if (nord gt 0) then begin
       ...
       for n = nordmx,0,-1 do begin
          tmp_ra=fdf_predict.poly[n]+(tref2-fdfpre_ref)*tmp_ra
       endfor
...
;   interpolate model to data - perform outlier rejection
    ...
    nnnn = n_elements(bx_sc.y) - 1L
    ;bx_sc.y = smooth(bx_sc.y,7 < nnnn,/edge)
    ;by_sc.y = smooth(by_sc.y,7 < nnnn,/edge)
    ;bz_sc.y = smooth(bz_sc.y,7 < nnnn,/edge)
    bx_sc.y = smooth(bx_sc.y,7 < nnnn,/edge_truncate)
    by_sc.y = smooth(by_sc.y,7 < nnnn,/edge_truncate)
    bz_sc.y = smooth(bz_sc.y,7 < nnnn,/edge_truncate)
    ...
    if (rem_spin_har and develop) then begin
       ...
       get_data,'BFIX',data=fix_res
       nnnn = n_elements(fix_res.y(*,0)) - 1L
;       fix_res.y(*,0) = smooth(fix_res.y(*,0),7 < nnnn,/edge)
;       fix_res.y(*,1) = smooth(fix_res.y(*,1),7 < nnnn,/edge)
;       fix_res.y(*,2) = smooth(fix_res.y(*,2),7 < nnnn,/edge)
       fix_res.y(*,0) = smooth(fix_res.y(*,0),7 < nnnn,/edge_truncate)
       fix_res.y(*,1) = smooth(fix_res.y(*,1),7 < nnnn,/edge_truncate)
       fix_res.y(*,2) = smooth(fix_res.y(*,2),7 < nnnn,/edge_truncate)
       store_data,'BFIX',data=fix_res
       fix_res=0
