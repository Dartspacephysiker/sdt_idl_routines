...
FUNCTION datetimesec, stringin
   ...
   car1=0
   IF STRPOS (stringin, ':') GE 0 THEN  $
       secsTime = timesec(stringin, carries=car1)

   IF secsDate GT 0.D THEN secsOut = secsOut + secsDate

   IF secsTime GT 0.D THEN BEGIN
       secsOut = secsOut + secsTime + (car1 * 86400)
    endif else begin
    endelse
