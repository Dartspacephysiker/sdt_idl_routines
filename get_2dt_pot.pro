...
;               15/08/22/       Made pot version (SMH, Dartmouth)
;NOTES:	  
;	Current version only works for FAST
;-
pro get_2dt_pot,funct,get_dat, $
	T1=t1, $
	T2=t2, $
	ENERGY=en, $
	ERANGE=er, $
...
if keyword_set(sc_pot) then begin
	if routine EQ 'get_fa_ees' or routine EQ 'get_fa_eeb' then begin
		
			sc_pot_min=min(abs(dat.time-sc_pot.x),ind)
...
sum = call_function(funct,dat,ENERGY=energy_sc_pot,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins)
